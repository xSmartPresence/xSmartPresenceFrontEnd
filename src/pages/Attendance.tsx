    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">Manage and review employee attendance</p>
        </div>
        {activeTab === "table" && (
          <div className="relative inline-block">
            <DatePicker
              ...
              customInput={
                <button type="button" className="flex items-center gap-3 bg-white border border-gray-300 rounded-xl px-5 py-2.5 shadow-sm hover:shadow-md transition">
                  <Calendar size={18} className="text-gray-600" />
                  <span className="font-medium text-gray-700">{format(selectedDate, "dd-MM-yyyy")}</span>
                </button>
              }
            />
          </div>
      ...
      <div className="flex gap-1 mb-6 border-b border-gray-200">
        {(["table", "calendar"] as const).map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === "table" ? "Table View" : "Calendar View"}
          </button>
        ))}
      </div>
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-x-auto">
            {loading ? (
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {records.map((r) => (
                    <tr key={`${r.code}-${r.date}`} className="border-b border-gray-200 hover:bg-gray-50">
                      <td className="px-4 py-3 whitespace-nowrap font-medium text-gray-900">{r.code}</td>

...
const [savingHoliday, setSavingHoliday] = useState(false);
  ...
  confidence_threshold: settings.confidenceThreshold / 100, // 80 → 0.80
  ...
  const handleCreateAdmin = async () => {
    ...
    await createAdminUser(adminForm);   // ✅ don't trust response
...
>
  ...
  <button
    ...
    onClick={(e) => {
      e.stopPropagation();
      setHolidayTypeOpen(!holidayTypeOpen);
    }}
    ...
    >
      {["Public", "Company"].map((type) => (
        <div
          key={type}
          onClick={() => {
            setHolidayForm({ ...holidayForm, type });
            setHolidayTypeOpen(false);
          }}
  ...
  <button
    ...
    onClick={(e) => {
      e.stopPropagation();
      setAdminRoleOpen(!adminRoleOpen);
    }}
    ...
    >
      {[
        ...
        <div
          key={role.value}
          onClick={() => {
            setAdminForm({ ...adminForm, role: role.value });
            setAdminRoleOpen(false);
          }}
          className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-sm"
        >
